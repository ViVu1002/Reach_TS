	...
	<div class="container">
		<nav aria-label="breadcrumb">
		  <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Thiết kế đồ họa 2D</li>
		  </ol>
		</nav>
		...
		<div class="row">
            <div class="col col-3">
                <ul style="display: block;" class="nav_top_loai_tin">NGÀNH HỌC
                    <li class="nav_loai_tin" style="border-left: 5px solid #fb9d0a; color: #fb9d0a;background: #fff;">Thiết kế đồ họa 2D</li>
            ...
            @if(isset($dohoa2s))
            <div class="col col-9" style="font-size: 16px">
                @foreach($dohoa2s as $dohoa)
                <img src="{{ asset('upload/tintuc').'/'.$dohoa->tt_avatar }}" style="margin-top: -15px" alt="ảnh" title="Lập trình Web">
                <h1 class="loai_tin_top_content">{{$dohoa->tt_name}}</h1>
                <p style="margin-top: 40px">{{$dohoa->tt_title_seo}}</p>
                <p>{!! $dohoa->tt_content !!}</p>
                @endforeach
			</div>
            @endif
		</div>
